...
|---|---|---|---|
| `paper` | Page background, default node fill | `#e6edf7` (cloud) | `#12172b` |
| `paper-2` | Diagram container bg, secondary fill | `#d9e2f0` (mist) | `#1a2140` |
| `ink` | Primary text, primary stroke | `#243060` (navy) | `#e6edf7` (cloud) |
| `muted` | Secondary text, default arrow stroke | `#4a5680` (ink-muted) | `#a9b6d6` |
| `soft` | Sublabels, boundary labels | `#53628b` (ink-soft) | `#78859e` |
| `rule` | Hairline borders | `rgba(36,48,96,0.12)` | `rgba(230,237,247,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#d9e2f0` (mist) | `#3a4776` |
| `accent` | Focal / 1–2 max per diagram | `#fab71e` (gold) | `#fab71e` (gold) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(250,183,30,0.08)` | `rgba(250,183,30,0.10)` |
| `link` | HTTP/API calls, external arrows | `#3662ab` (royal) | `#6889c0` |

> **Brand palette source:** this skin maps to the Sentina brand palette (`sentina-landing/brand-kit/tokens/tokens.css`) — `navy #243060`, `royal #3662ab`, `sky #2fa3df`, `deep #1d417f`, `gold #fab71e`, `cloud #e6edf7`, `mist #d9e2f0`, `ink-muted #4a5680`, `ink-soft #53628b` (WCAG-AA-tuned from the brand's `#5b6b93`). Gold is a deliberate pick for `accent`: the brand's own rule caps gold at ~10% usage for emphasis only, which is exactly this tool's "1–2 focal nodes max per diagram" role, not a coincidence. `link` uses royal, the brand's documented interactive/link color. Dark-mode values reuse the same derived dark palette established for the Sentina Obsidian theme, for consistency across the whole design system.
...
| `node-name` | Nunito Sans | 12px | 600 | Human-readable labels |
...
Sentina's brand typography is Montserrat (headings) + Nunito Sans (body), both sans, no serif. Per this file's own onboarding constraint ("if brand typography is all sans, keep Instrument Serif for `title` and `callout` anyway, the contrast is load-bearing"), `title`/`callout` stay Instrument Serif rather than switching to Montserrat, and `sublabel`/`eyebrow`/`arrow-label` stay Geist Mono (brand defines no monospace, and this file separately warns against generic mono substitutions). Only `node-name` — the actual body/label sans role — maps to the brand's Nunito Sans.

### Font stack

```html
<link href="https://fonts.googleapis.com/css2?family=Instrument+Serif:ital@0;1&family=Nunito+Sans:wght@400;600;700&family=Geist+Mono:wght@400;500;600&family=Noto+Sans+KR:wght@400;500;600&family=Noto+Serif+KR:wght@400&family=Noto+Sans+TC:wght@400;500;600&family=Noto+Serif+TC:wght@400&display=swap" rel="stylesheet">
```

### Korean labels

Nunito Sans and Instrument Serif carry no Hangul. A Korean `<text>` element extends its own family — never swap the skin:

```svg
<text font-family="'Nunito Sans', 'Noto Sans KR', 'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif">결제 서비스</text>
```
...
**Load-bearing rule:** Mono is for *technical* content (ports, commands, URLs, field types). Names go in Nunito Sans. Page title is Instrument Serif. Italic Instrument Serif is reserved for annotation callouts (see [primitive-annotation.md](primitive-annotation.md)). **Never JetBrains Mono** as a blanket "dev" font.

### Traditional Chinese labels

Nunito Sans and Instrument Serif carry no Han. A Traditional Chinese `<text>` element extends its own family — never swap the skin:

```svg
<text font-family="'Nunito Sans', 'Noto Sans TC', 'PingFang TC', 'Microsoft JhengHei', sans-serif">請求項比對</text>
```
