...
| `line`      | N/A   | Non-directional connections, dividers  | ❌                   | Binds to shapes |
| `text`      | A     | Standalone labels, titles, annotations | N/A                  | Not bindable    |
...
Arrow labels also require `boundElements` on the arrow plus a separate text element with `containerId`. **Never use `label: { text: "..." }` on arrows.**
...
**Element order with zones:** zones, then shapes, then arrows, then text elements
...
**Mixed mode:** Use `roughness: 0` for background zones and `roughness: 1` for shapes, since zones feel like structure and shapes feel like content.
