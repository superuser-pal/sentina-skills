...
**Correct approach:** Add `boundElements` to the shape, then create a **separate text element** that references the shape via `containerId`. Always declare shapes first, arrows second, and text elements last, which ensures text renders on top and is never obscured by arrows.
...
| **Canvas**            | White       | N/A          | `#ffffff` | N/A        |
| **Default stroke**    | N/A         | Near-black   | N/A       | `#1e1e1e`  |
...
| **Default stroke** | N/A        | White       | N/A       | `#ffffff`  |
...
Background zones clean (`roughness: 0`) plus foreground shapes sketchy (`roughness: 1`):

- Zones: `roughness: 0`, structural and precise
- Shapes: `roughness: 1`, content and approachable
...
- `opacity: 35`: semi-transparent (20-40 range works; 35 is the sweet spot)
...
| Ellipse   | Short text (circle)      | 120×120px | N/A       |
| Ellipse   | Longer text              | 160×120px | N/A       |
| Diamond   | Short question           | 140×140px | N/A       |
| Diamond   | Longer question          | 180×180px | N/A       |
...
A flowchart with two connected shapes. Note the element order: shapes, then arrows, then text elements (this ensures text is always rendered on top, never obscured by arrows).
