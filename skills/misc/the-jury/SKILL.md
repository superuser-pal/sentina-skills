---
name: the-jury
description: Convenes a panel of 3-5 subagent jurors that form independent blind opinions, deliberate anonymously, and return one committed verdict with confidence and preserved dissent.
disable-model-invocation: true
license: CC-BY-4.0
argument-hint: "the question, decision, or tradeoff that needs a verdict"
metadata:
...
This skill is user-invoked on purpose: the repo already has `doubt-driven-development` as an auto-invoked fresh-context adversarial reviewer for decisions. Reach for this skill by name when you specifically want a multi-persona panel that deliberates and commits to a verdict, rather than a single reviewer surfacing issues.
