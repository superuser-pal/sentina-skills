...
const THEMES = {
  ...
  'sentina': { label: 'Sentina', mode: 'light', css: {
    '--bg':'#e6edf7', '--surface':'#d9e2f0', '--surface-elevated':'#ffffff',
    '--border':'rgba(36,48,96,.08)', '--border-bright':'rgba(36,48,96,.15)',
    '--text':'#4a5680', '--text-dim':'#53628b',
    '--accent':'#3662ab', '--accent-dim':'rgba(54,98,171,.12)',
    '--node-a':'#3662ab', '--node-a-dim':'rgba(54,98,171,.12)',
    '--node-b':'#2fa3df', '--node-b-dim':'rgba(47,163,223,.12)',
    '--node-c':'#1d417f', '--node-c-dim':'rgba(29,65,127,.12)',
    '--green':'#1a7f37', '--green-dim':'rgba(26,127,55,.10)',
    '--red':'#cf222e', '--red-dim':'rgba(207,34,46,.10)',
    '--orange':'#bc4c00', '--orange-dim':'rgba(188,76,0,.10)'
  } }
...
const FONT_PAIRS = {
  ...
  'sentina':    { label: 'Sentina (Montserrat)', sans: "'Montserrat'",         mono: "'JetBrains Mono'" }
};
```

Sentina's brand pairs Montserrat for headings with Nunito Sans for body (see `sentina-landing/brand-kit/tokens/tokens.css`), but this picker has only one `sans` role shared by headings and body, the same tradeoff the `instrument` pair already makes. Montserrat is used here since generated pages (diagrams, dashboards, slide decks) lean label- and headline-heavy rather than long-form prose.
...
```js
const DEFAULT_THEME = 'sentina';
const DEFAULT_FONT = 'sentina';
