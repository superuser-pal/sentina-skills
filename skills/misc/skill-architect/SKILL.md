...
description: Guides you through building a high-quality skill from scratch via structured conversation, covering discovery, architecture, craft, validation, and delivery.
disable-model-invocation: true
license: CC-BY-4.0
argument-hint: "the workflow or task you want to turn into a skill"
metadata:
...
This skill is user-invoked on purpose: the repo also has `writing-for-agents` for the mechanics of writing any agent-consumed document (skills, `AGENTS.md`, `CLAUDE.md`). `writing-for-agents` stays the auto-invoked reference for quick edits; reach for this skill when you want the full guided-discovery conversation for building a skill from scratch.
...
- Lean slightly "pushy": agents tend to undertrigger, so it's better to load
  and not need it than to miss a relevant query.
...
- **Never wrap prose lines at arbitrary column widths** (e.g. 80 chars). Let each sentence or paragraph be a single long line. Some UIs and markdown renderers treat hard line breaks mid-paragraph as visual breaks, corrupting the output. Code blocks are exempt, since those can wrap for readability.
...
Share the packaged skill with a brief summary:
...
- If a formal eval/benchmark pass is wanted later, ask about a skill-evaluation tool if one is available in the current environment
...
- Match the user's technical level, explaining terms if they seem non-technical
- Be direct about tradeoffs: if a design choice has a downside, say so

## Important Boundaries

- This skill is for CREATING new skills. If the user wants to build a subagent/persona
  definition instead, point them at the `subagent-creator` skill.
