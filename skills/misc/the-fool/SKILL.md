---
name: the-fool
description: Plays devil's advocate against an idea, plan, or decision, running a pre-mortem, red team, or Socratic challenge to find blind spots before you commit.
disable-model-invocation: true
license: CC-BY-4.0
argument-hint: "the plan, decision, or proposal to challenge"
metadata:
  author: "https://github.com/Jeffallan, adapted for Sentina from tech-leads-club/agent-skills"
  version: "2.0.0"
---
...
This skill is user-invoked on purpose: the repo already has `doubt-driven-development` (fresh-context adversarial review before a decision stands) and `grilling` (interviewing the user to reach shared understanding) as auto-invoked skills covering adjacent ground. Reach for this skill by name when you specifically want a single agent applying named critique frameworks (Socratic, pre-mortem, red team, dialectic, falsification) against an already-formed position. For a multi-agent panel that challenges from several angles and then commits to a verdict, use `the-jury` instead.
...
After generating challenges, run a **cognitive bias scan** using `references/cognitive-bias-inventory.md` to flag any biases present in the user's reasoning. Weave bias findings into your challenges rather than presenting them as a separate section.
...
After synthesis, offer a second pass with a different mode if warranted, consulting `references/mode-selection-guide.md` for recommended multi-mode sequences.
